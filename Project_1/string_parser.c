...
/*EX
INPUT
char *input = "cd folder";           // a sample input line
const char *delimiter = " ";         // split by space
command_line result = str_filler(input, delimiter);

OUTPUT
result.num_token = 2;
result.command_list[0] = "cd";
result.command_list[1] = "folder";
result.command_list[2] = NULL;  // always null-terminated

ACCESS TOKENS LIKE
for (int i = 0; result.command_list[i] != NULL; i++) {
    printf("Token %d: %s\n", i + 1, result.command_list[i]);
}
*/
